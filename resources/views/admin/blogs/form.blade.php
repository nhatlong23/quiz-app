                ...
                <div class="tab-content">
                    <div class="tab-pane tabs-animation fade show active" id="tab-content-1" role="tabpanel">
                        <div class="main-card mb-3 card">
                            <div class="card-body">
                                @if (!isset($blogs))
                                    <form method="POST" enctype="multipart/form-data" id="myForm" action="{{ route('blogs.store') }}">
                                    @else
                                        <form method="POST" enctype="multipart/form-data" action="{{ route('blogs.update', $blogs->id) }}">
                                            @method('PUT')
                                @endif
                                @csrf
                                ...
                                <div class="position-relative row form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="status" class="form-control">
                                            <option value="1"
                                                {{ isset($blogs) && $blogs->status == 1 ? 'selected' : '' }}>Hiển thị blog
                                            </option>
                                            <option value="0"
                                                {{ isset($blogs) && $blogs->status == 0 ? 'selected' : '' }}>Ẩn blog
                                            </option>
                                        </select>
                                    </div>
                                </div>
